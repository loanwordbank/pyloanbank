...
- **`segment_slice_alignment_columns`** — Optional tuple of cognate alignment column names. After **`_cmd_makecldf_body`**, empty **`Segment_Slice`** values are filled from the linked form's **`Segments`** phoneme prefix when a surplus ``+`` appears in an alignment (string or token list). Set e.g. **`("Uralign",)`** on UEW-hu or **`("Monogap",)`** on WestOldTurkic. Alignment column descriptions in **`etc/ExtraCols.toml`** should document the ``+`` convention; the standard **`Segment_Slice`** column description is patched automatically.
